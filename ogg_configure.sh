...
install_prefix=`pwd`/ogg-install-$platform-$arch
...
build_prefix=`pwd`/ogg-build-$platform-$arch
...
cd ..
resultSpeexPah=`pwd`/ogg  #最后生成的speex

if [ !  -d "$resultSpeexPah" ]; then
 mkdir "$resultSpeexPah"
 cp -f $install_prefix/lib/libogg.a $resultSpeexPah/libogg.a
else
	echo "合并"
	lipo=`xcrun -find lipo`
	lipo -create $install_prefix/lib/libogg.a  $resultSpeexPah/libogg.a -output $resultSpeexPah/libogg.a  
fi
